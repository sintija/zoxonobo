...
export const setupPageAnimations = (videoElement, aboutElement, containerElement, festivalInfoElement) => {
    if (!videoElement || !aboutElement || !containerElement) return;

    // Get current breakpoint
    const currentBreakpoint = getBreakpoint();
    console.log(`Current breakpoint: ${currentBreakpoint}, width: ${window.innerWidth}px`);
    ...
    gsap.set(aboutElement, { scale: animationConfig.initialScale[currentBreakpoint] });
    ...
    thirdTl.fromTo(imagesContainer,
        { x: "0" },
        {
    ...
    // Fourth Animation - Festival Info Transition
    const fourthTl = gsap.timeline({
        scrollTrigger: {
            ...
            scrub: 1,
            markers: true,
            id: "festival-info-animation",
            preventOverlaps: true,
            fastScrollEnd: true
        }
    });

    fourthTl.to(imagesSection, {
        opacity: 0,
        duration: 0.8,
        ease: "power2.inOut",
    ...
    const festivalInfoContainer = festivalInfoElement.closest('.festival-info-container');

    // Festival info fades in
    fourthTl.to(festivalInfoContainer, {
        opacity: 1,
        visibility: "visible",
        duration: 0.8,
        ease: "power2.inOut",
    ...
    if (festivalInfoElement) {
        const heading = festivalInfoElement.querySelector('h2');
        const paragraph = festivalInfoElement.querySelector('p');

        gsap.set([heading, paragraph], { opacity: 0 });
